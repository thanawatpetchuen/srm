...
class SSP {
	...
	static function complex ( $request, $conn, $table, $primaryKey, $columns, $whereResult=null, $whereAll=null, $beforeFrom=null )
	{
		...
		$has_where = strpos($table,'WHERE');

		if ($has_where !== false) {
			if ($where !== '') {
				$clause = "SELECT " . implode(", ", self::pluck($columns, 'db')) . " FROM $table AND $where $order $limit";
			} else {
				$clause = "SELECT " . implode(", ", self::pluck($columns, 'db')) . " FROM $table $where $order $limit";
			}
		} else {
			if ($where !== '') {
				$clause = "SELECT " . implode(", ", self::pluck($columns, 'db')) . " FROM $table WHERE $where $order $limit";
			} else {
				$clause = "SELECT " . implode(", ", self::pluck($columns, 'db')) . " FROM $table $where $order $limit";
			}
		}
