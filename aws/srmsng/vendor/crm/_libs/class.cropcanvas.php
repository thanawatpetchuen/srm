...
class CropCanvas
{
	...
	function bulidImage($type = 'png', $quality = 90){
        if ($this->_imgFinal == null) {
            $this->_debug('There is no cropped image to show.');
            return false;
        }
        ...
        switch($type) {
            ...
            case 'png':
				$func($this->_imgFinal, "crop_file.png", 100);
                break;
            case 'jpg': 
				$func($this->_imgFinal, "images/crop_file.jpg", 100);
                break;
        }
    ...
    function _cropSize($ox, $oy, $nx, $ny, $position){
		...
		imagejpeg($this->_imgFinal, $GLOBALS['_cropImageName']);
