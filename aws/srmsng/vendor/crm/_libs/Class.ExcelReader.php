...
class Spreadsheet_Excel_Reader {
	...
	function datasheet($sheet=0){
		$_date;
		//echo "colcount >>".$this->colcount($sheet);
		//echo "rowcount >>".$this->rowcount($sheet);
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				$_date[$col][$row] = trim(iconv("UTF-8", "TIS-620", $this->val($row,$col,$sheet)));
			}
		}
		return $_date;
	}
	function getColCount($sheet=0){
		return $this->colcount($sheet);
	}
	function getRowcount($sheet=0){
